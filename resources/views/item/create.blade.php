    ...
    <form action="{{ route('item.store') }}" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 35px;">
                ...
                <div style="position: relative;">
                    <select name="condition_id" id="condition_id" style="width: 100%; border: 1px solid #ccc; border-radius: 4px; padding: 10px; font-size: 0.95em; color: #333; background-color: #fff; appearance: none; -webkit-appearance: none;">
                        ...
                        @foreach($conditions as $condition)
                            <!-- 💡 カラム名を $condition->condition に修正しました -->
        ...
        <div>
            ...
            <div style="margin-bottom: 20px;">
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" style="width: 100%; border: 1px solid #ccc; border-radius: 4px; padding: 10px; font-size: 0.95em; box-sizing: border-box;">
            </div>

            <!-- ブランド名 -->
            <div style="margin-bottom: 20px;">
                <label for="brand" style="display: block; font-weight: bold; margin-bottom: 8px; color: #333;">ブランド名</label>
                <input type="text" name="brand" id="brand" value="{{ old('brand') }}" style="width: 100%; border: 1px solid #ccc; border-radius: 4px; padding: 10px; font-size: 0.95em; box-sizing: border-box;">
            </div>

            <!-- 商品の説明 -->
            <div style="margin-bottom: 20px;">
                <label for="description" style="display: block; font-weight: bold; margin-bottom: 8px; color: #333;">商品の説明</label>
                <textarea name="description" id="description" rows="5" style="width: 100%; border: 1px solid #ccc; border-radius: 4px; padding: 10px; font-size: 0.95em; box-sizing: border-box; resize: vertical;">{{ old('description') }}</textarea>
            </div>
            ...
            <div>
                ...
                <div style="position: relative; display: flex; align-items: center;">
                    <span style="position: absolute; left: 12px; font-size: 1.1em; font-weight: bold; color: #333;">¥</span>
                    <input type="number" name="price" id="price" value="{{ old('price') }}" style="width: 100%; border: 1px solid #ccc; border-radius: 4px; padding: 10px 10px 10px 30px; font-size: 0.95em; box-sizing: border-box;">
                </div>
            </div>
        </div>
...
<!-- 💡 スキル・インタラクション制御用のスクリプト -->
<!-- 💡 ファイルの一番最後に貼り付けます -->
<script>
    // 画面読み込み時に、すでにチェックが入っているカテゴリーを赤く塗りつぶす
    ...
    // 💡 選択された画像を読み込んで、枠内にパッと表示させる関数（ここに追加！）
    function previewImage(input) {
        ...
        if (input.files && input.files[0]) {
            ...
            // 2. 画像ファイルを読み込んでプレビューにセット
            const reader = new FileReader();
            reader.onload = function(e) {
                preview.src = e.target.result;
                preview.style.display = 'block'; // 隠していた画像タグを表示！
            }
