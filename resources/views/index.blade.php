                ...
                <div style="position: relative;">
                    ...
                    <a href="{{ route('item.show', $item->id) }}" style="text-decoration: none; color: inherit; display: block;">
                        ...
                        <div style="position: relative; width: 100%; aspect-ratio: 1 / 1; background-color: #e5e5e5; border-radius: 4px; overflow: hidden; margin-bottom: 10px;">
                            ...
                            <img src="{{ str_starts_with($item->image_path, 'http') ? $item->image_path : asset('storage/' . $item->image_path) }}" alt="{{ $item->name }}" style="width: 100%; height: 100%; object-fit: cover; transition: opacity 0.2s; {{ $item->is_sold ? 'filter: grayscale(40%); opacity: 0.7;' : '' }}">
