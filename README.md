...
* **背景と自論**:初期設計では、ログインや会員登録画面においてエラーメッセージの表示位置やスタイルが異なっていました。また、Laravelの仕様上「確認用パスワードの不一致エラー」がメインのパスワード欄と紐づくため、確認用入力欄の近くにエラーが出にくいというUI/UX上の課題がありました。画面ごとにエラーの出し方がバラバラでは、ユーザーが操作ミスをした際にどこを確認すればよいか瞬時に判断できず、ストレスを与えてしまうと考えました。
* **改善点**:アプリ全体の視認性を統一するため、認証関係の画面において「エラーメッセージはすべてフォーム上部の専用エリアに一括でリスト表示する」スタイルへと統一しました。コントローラーのバリデーションルールに confirmed を明示的に組み込むバックエンドの改修を行い、確認パスワードの一致エラーも含めてすべての日本語エラーが最上部へ一列に揃うロジックを実装しました。これにより、ユーザーが「何が原因で進めないのか」をひと目で網羅できる、迷わないフォームUIを実現しました。
